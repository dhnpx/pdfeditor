...
pub fn main() !void {
    ...
    defer if (general_purpose_allocator.deinit() != .ok) @panic("Memory leak on exit!");
    ...
    main_loop: while (true) {
        // beginWait coordinate with waitTime to run frames only when needed
        const nstime = window.beginWait(backend.hasEvent());

        // marks the beginning of a frame for dvui, can call dvui functions after this
        try window.begin(nstime);

        // sends all SDL events to dvui for processing
        const quit = try backend.addAllEvents(&window);
        ...
        backend.textInputRect(window.textInputRequested());
        ...
        if (show_dialog_outside_frame) {
            ...
            try dvui.dialog(@src(), .{ .window = &window, .modal = false, .title = "Dialog from Outside", .message = "this is a non modal dialog that was created outisde window.begin()/window.end(), usually from another thread." });
        }
    }
}
...
fn gui_frame() !void {
    ...
    {
        var menu = try dvui.menu(@src(), .horizontal, .{ .background = true, .expand = .horizontal });
        defer menu.deinit();
